...
export const useLmsNotificationTugasStore = defineStore('LmsNotificationTugasStore', {
  state: () => ({
    tab: 'index',
    keyword: '',
    init: {
  ...
  actions: {
    ...
    async onIndex(page = null) {
      ...
      await onRequestPrivate(this, {
        url: host + '/lms/notifications/tugas',
        method: 'get',
        params: params,
      }, this.tab)
      Loading.hide()
    ...
    async onReadAll() {
      ...
      const temp = await onRequestPrivate(this, {
        url: host + '/lms/notifications/tugas/read-all',
        method: 'post',
      }, 'index')
      Loading.hide();
      if(temp) {
    ...
    async onRead(id = 1) {
      ...
      const data = await onRequestEditPrivate(this, {
        url: host + `/lms/notifications/tugas/${id}/read`,
        method: 'post'
      }, 'read')
      Loading.hide()
    ...
    async onDelete() {
      ...
      Loading.show();
      const data = await onRequestEditPrivate(this, {
        url: host + `/lms/notifications/tugas/${id}`,
        method: 'delete'
      }, 'read')
      Loading.hide();
    ...
    async onShow(slug = null) {
      ...
      Loading.show()
      await onRequestPrivate(this, {
        url: host + '/lms/notifications/tugas/'+slug,
        method: 'get',
      }, 'show')
      Loading.hide()
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useLmsNotificationTugasStore, import.meta.hot))
}
