...
import { useLmsSiswaStore } from "src/stores/lms/LmsSiswaStore.js";
...
export default {
  ...
  methods: {
    ...
    ...mapActions(useLmsSiswaStore, ["onShow"]),
    async onOpen(id) {
      this.dialog = true;
      this.id = id;
      ...
      // console.log("ref?.siswa?.parents", ref?.siswa?.parents.map(item => item.parent.id), ref);
      ...
      this.form_edit['ortu_id'] = form_edit?.siswa?.parents.map(item => item.parent.id)
