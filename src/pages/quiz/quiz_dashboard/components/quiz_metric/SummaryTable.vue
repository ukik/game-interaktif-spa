<template>
  <q-card>

    <q-card-section>
      <div class="text-h6">Dashboard</div>
    </q-card-section>

    <q-separator></q-separator>

    <q-list separator class="row bg-white">
      <div class="col-xs-12 col-sm-6">
        <q-item>
          <q-item-section avatar>
            <q-avatar color="primary" text-color="white" icon="account_circle" />
          </q-item-section>

          <q-item-section>Total User</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_users ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <div class="col-xs-12 col-sm-6 row">
        <q-separator vertical></q-separator>
        <q-item class="col">
          <q-item-section avatar>
            <q-avatar color="primary" text-color="white" icon="account_circle" />
          </q-item-section>

          <q-item-section>Total Guru</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_teachers ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <div class="col-xs-12 col-sm-6">
        <q-item>
          <q-item-section avatar>
            <q-avatar color="cyan" text-color="white" icon="assignment_ind" />
          </q-item-section>

          <q-item-section>Total Siswa</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_students ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <div class="col-xs-12 col-sm-6 row">
        <q-separator vertical></q-separator>
        <q-item class="col">
          <q-item-section avatar>
            <q-avatar color="cyan" text-color="white" icon="assignment_ind" />
          </q-item-section>

          <q-item-section>Total Ortu</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_parents ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <div class="col-xs-12 col-sm-6">
        <q-item>
          <q-item-section avatar>
            <q-avatar color="orange" text-color="white" icon="assignment" />
          </q-item-section>

          <q-item-section>Total Tugas</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_tugas ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <!-- <div class="col-xs-12 col-sm-6 row">
        <q-separator vertical></q-separator>
        <q-item class="col">
          <q-item-section avatar>
            <q-avatar color="orange" text-color="white" icon="assignment" />
          </q-item-section>

          <q-item-section>Total Modul</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_modul ?? 0) }}</q-item-section>
        </q-item>
      </div> -->
      <div class="col-xs-12 col-sm-6 row">
        <q-separator vertical></q-separator>
        <q-item class="col">
          <q-item-section avatar>
            <q-avatar color="pink" text-color="white" icon="assignment_late" />
          </q-item-section>

          <q-item-section>Total Mapel</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_mapel_used ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <div class="col-xs-12 col-sm-6">
        <q-item>
          <q-item-section avatar>
            <q-avatar color="pink" text-color="white" icon="assignment_late" />
          </q-item-section>

          <q-item-section>Total Quiz</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_quiz ?? 0) }}</q-item-section>
        </q-item>
      </div>
      <div class="col-xs-12 col-sm-6 row">
        <q-separator vertical></q-separator>
        <!-- <q-item class="col">
          <q-item-section avatar>
            <q-avatar color="orange" text-color="white" icon="assignment" />
          </q-item-section>

          <q-item-section>Total Modul</q-item-section>
          <q-item-section side class="text-h6">{{ Number(item?.total_modul ?? 0) }}</q-item-section>
        </q-item> -->
      </div>
    </q-list>
  </q-card>
</template>

<script>
export default {
  props: ['item'],

}

</script>
