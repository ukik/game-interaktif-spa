<template>
  ...
  <q-card v-if="items?.length > 0">
    <q-card-section>
      <div class="text-h6">Score Modul Distribution by Kelas</div>
    </q-card-section>
    ...
    <q-markup-table separator="vertical" class="">
      ...
      <tbody>
        ...
        <tr v-if="items" v-for="(item, index) in items">
          <td>
            <template v-if="item?.kelas_id">
              <q-badge v-for="($j) in item?.kelas_id?.toString()?.split(',')" class="badge bg-secondary text-white p-2 mb-1">{{ getNamaKelas($j) }}</q-badge>
            </template>
            <span v-else>-</span>
          </td>
