<template>
  ...
  >
    ...
    <q-card
      v-if="get_aktivitas_tugasable?.kategori"
      class="card-shadow card-border-radius game"
    >
      <q-card-section class="bg-pink text-center">
        <div v-if="get_aktivitas_tugasable?.mapel?.nama" class="title text-white">
          {{ get_aktivitas_tugasable?.mapel?.nama }}
        </div>
        <div v-if="get_aktivitas_tugasable?.judul" class="subtitle text-white">
          {{ get_aktivitas_tugasable?.judul }}
        </div>
      </q-card-section>
        ...
        <q-card flat bordered class="stats">
          <template v-for="(item, index) in get_aktivitas_payload?.intro?.konten">
            <div class="text-body2">
              ...
              <span class="text-body2">{{ item?.value }}</span>
            </div>
          </template>
        </q-card>

        <HintRender class="q-mt-md">
...
import HintRender from "../components/HintRender.vue";
