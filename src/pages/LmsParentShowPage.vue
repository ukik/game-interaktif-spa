          ...
          <q-card-section v-if="get_show_payload?.parent?.siswa && get_show_payload?.parent?.siswa.length > 0"
            ...
            <q-list separator bordered class="text-dark">
              <q-item v-for="(item, index) in get_show_payload?.parent?.siswa" :key="index"
                :to="{ name: 'lms-siswa-show', params: { slug: item?.user_id } }" clickable v-ripple>
                <q-item-section avatar>
                  <q-avatar size="80px">
...
export default {
  ...
  computed: {
    ...
    ...mapState(useLmsParentStore, ["get_show_payload", "get_init_show"]),
  },
  methods: {
    ...mapActions(useAuthStore, ["onLogout"]),
    ...mapActions(useLmsParentStore, ["onShow", "onChangePage"]),
    onBubbleEvent(val) {
