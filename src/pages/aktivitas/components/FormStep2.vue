<template>
  <div class="q-gutter-md">
    ...
    <div class="q-mb-lg">
      <q-select
        clearable
        v-model="form_tugas_edit.kelas_id"
        :options="get_kelas_payload"
        option-label="nama"
        option-value="id"
        emit-value
        map-options
    ...
    <q-field
      label="Siswa"
      stack-label
      :model-value="form_tugas_edit.siswa_ids"
      hint="required"
      :rules="[(val) => val.length > 0 || 'Minimal pilih 1 siswa']"
      borderless
      no-error-icon
    >
      <!-- LABEL CUSTOM -->
      <template v-slot:label>
        <div class="text-weight-medium text-grey-8 q-mb-lg">Pilih Siswa</div>
      ...
      <template v-slot:control>
        <q-list class="col-12" bordered separator>
          ...
          <q-item
            v-if="get_siswa_payload?.data.length > 0"
            v-for="(item, index) in get_siswa_payload?.data"
            tag="label"
            v-ripple
          >
            <q-item-section avatar>
              <q-checkbox
                v-model="form_tugas_edit.siswa_ids"
                :val="item?.siswa?.id"
                color="teal"
              />
            </q-item-section>
...
export default {
  ...
  computed: {
    ...
    isAllChecked() {
      ...
      return list.length > 0 && list.every((item) => selected.includes(item.siswa?.id));
    },
  },
  watch: {
    ...
    // },
    "form_tugas_edit.siswa_ids": {
      deep: true,
      handler(val) {
        // console.log("form_tugas_edit.siswa_ids", val);
        let temp = [];
        this.get_siswa_payload?.data?.forEach((el) => {
          val.forEach((id) => {
            if(el?.siswa?.id == id) {
              temp.push(el)
            }
        ...
        console.log('form_tugas_edit.siswa_ids',temp)
        this.form_tugas_edit['tugas_siswa'] = temp
      },
    },
  },
  methods: {
